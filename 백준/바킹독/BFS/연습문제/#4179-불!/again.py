...
def solution():
    ...
    temp_jido = [list(input()) for _ in range(rn - 2)]
    ...
    for r in range(rn - 2):
        for c in range(cn - 2):
            if temp_jido[r][c] == JIHUN:
                jido[r + 1][c + 1] = BLANK
                visit_t[r + 1][c + 1] = True
                jq.append((r + 1, c + 1, 0))
            elif temp_jido[r][c] == FIRE:
                jido[r + 1][c + 1] = 0
                fq.append((r + 1, c + 1))
            else:
                jido[r + 1][c + 1] = temp_jido[r][c]
    ...
    while fq:
        ...
        for dr, dc in ds:
            newr, newc = r + dr, c + dc
            if not (1 <= newr < rn - 1 and 1 <= newc < cn - 1):
                continue
            if jido[newr][newc] == BLANK:
                jido[newr][newc] = jido[r][c] + 1
                fq.append((newr, newc))
    ...
    while jq:
        ...
        for dr, dc in ds:
            ...
            if not (1 <= newr < rn and 1 <= newc < cn - 1):
                continue
